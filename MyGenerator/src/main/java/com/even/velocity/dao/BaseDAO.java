...
import com.lottery.dao.constant.SqlId;
import com.lottery.exception.DaoException;
import com.lottery.utils.BeanUtils;
...
public abstract class BaseDAO<T extends Identifiable> {
    ...
    */
    ...
    public <V extends T> List<V> selectByColumn(T query) {
        try {
            Map<String, Object> params = BeanUtils.toMap(query);
            return sqlSessionTemplate.selectList(getSqlName(SqlId.SQL_SELECT_BY_COLUMN), params);
        } catch (Exception e) {
            throw new DaoException(String.format("查询对象列表出错！语句：%s", getSqlName(SqlId.SQL_SELECT)), e);
        }
    }
